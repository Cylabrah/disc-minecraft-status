...
const botToken = "NTcyNzIyODA3OTI3MDc4OTEz.XMgyeQ.kdd6aX2KCapF1L6mnhG__zObQcg";
const command = ".status";
const ip = "51.79.1.206:25629";
const pingWait = 60 * 1000; //** 60 seconds * 1000 miliseconds
...
function pingServer() {
    https.get("https://api.mcsrvstat.us/2/" + ip, function (res) {
        var body = "";
...
client.on("ready", () => {
    ...
    client.setInterval(pingServer, pingWait);
});

client.on("message", msg => {
	
	if (msg.author.username === 'sangre') {
		//** Harass Dominguez
		msg.author.sendMessage('shut up.');
	}
